...
public class NodoMenuServiceM extends FachadaService<NodoMenu> {
	...
	public String consultarNodoMenuPorRoles(
		...
		try {
			if (validarSesion(idSesion, accessToken)) {
				...
				List<NodoMenu> nodosHijos = nodoMenuDAO
						.findByRolInPermisoSeguridad(idRoles);
				...
				for (NodoMenu nodoMenuHoja : nodosHijos) {
					...
					Integer padre = nodoMenuHoja.getFkNodoMenu()
							.getIdNodoMenu();
					...
					while (padre != 0) {
						if (nodoMenuPadre == null) {
							throw new Exception(
									"Error Code: 555-Hay un error en la base de datos. Permisos inaccesibles (Servicios Web)");
						}
				...
				for (NodoMenu nodoMenuAUnir : listasDeNodosMenues) {
					nodoMenuPrincipal = unirListas(nodoMenuPrincipal,
							nodoMenuAUnir);
				}
	...
	private NodoMenu unirListas(NodoMenu nodoMenuPrincipal,
			NodoMenu nodoMenuAUnir) {
		List<NodoMenu> nodosMenuHijosDePrincipal = nodoMenuPrincipal.getHijos();

		nodoMenuAUnir = nodoMenuAUnir.getHijos().get(0);
		// Arbol izquierdo sin hijos? Entonces coloquele como hijo, el arbol
		// derecho.
		if (nodosMenuHijosDePrincipal == null
				|| nodosMenuHijosDePrincipal.isEmpty()) {
		...
		// Arbol izquierdo tiene como hijo al derecho? Adentrarse al arbol
		// recursivamente.
		for (NodoMenu nodoMenuHija : nodosMenuHijosDePrincipal) {
			if (nodoMenuHija.getIdNodoMenu().equals(
					nodoMenuAUnir.getIdNodoMenu())) {
		...
		// Arbol izquierdo NO tiene como hijo al derecho? Entonces unalo al
		// arbol izquierdo.
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathEliminarEnCascada(
			@PathParam("idSesion") Integer idSesion,
			@PathParam("accessToken") String accessToken,
	...
	public String eliminarEnCascada(Integer idSesion, Integer id)
			throws Exception {
	...
	public String consultarArbol(@PathParam("idSesion") Integer idSesion,
		...
		try {
			if (validarSesion(idSesion, accessToken)) {
				...
				NodoMenu nodoMenuPrincipal = new NodoMenuDAO().find(0);

				nodoMenuPrincipal.setHijos(buscarTodosLosHijos(
						nodoMenuPrincipal.getIdNodoMenu(), idRol));

				auditar(idSesion, getTable(), AccionEnum.CONSULTAR.ordinal(),
						0, String.valueOf(idRol));
	...
	private List<NodoMenu> buscarTodosLosHijos(Integer idNodoMenu, Integer idRol) {
		List<NodoMenu> hijos = new NodoMenuDAO().findByNodoMenu(idNodoMenu);

		for (NodoMenu nodoMenu : hijos) {
			setOperacionesDeVista(nodoMenu);
			setOperacionesDeNodoMenu(nodoMenu, idRol);

			nodoMenu.setHijos(buscarTodosLosHijos(nodoMenu.getIdNodoMenu(),
					idRol));
		}
	...
	private void setOperacionesDeVista(NodoMenu nodoMenu) {
		if (!nodoMenu.getTipoNodoMenuEnum()
				.equals(TipoNodoMenuEnum.TRANSACCION)) {
			return;
		}
		...
		List<VistaOperacionBasico> vistasOperacionesBasico = vistaOperacionBasicoDAO
				.findByVista(nodoMenu.getFkVista().getIdVista());

		for (VistaOperacionBasico vistaOperacionBasico : vistasOperacionesBasico) {
			Operacion operacion = new Operacion(
					vistaOperacionBasico.getOperacion(),
		...
		List<VistaOperacionCustom> vistasOperacionesCustom = vistaOperacionCustomDAO
				.findByVista(nodoMenu.getFkVista().getIdVista());

		for (VistaOperacionCustom vistaOperacionCustom : vistasOperacionesCustom) {
			Operacion operacion = new Operacion(
					vistaOperacionCustom.getOperacion(),
	...
	private void setOperacionesDeNodoMenu(NodoMenu nodoMenu, Integer idRol) {
		...
		List<PermisoSeguridad> permisoSeguridads = permisoSeguridadDAO
				.findByNodoMenuAndRoles(nodoMenu.getIdNodoMenu(),
		...
		for (PermisoSeguridad permisoSeguridad : permisoSeguridads) {
			Operacion operacion = OperacionHelper.getPorId(permisoSeguridad
					.getOperacion());

			Vista vista = permisoSeguridad.getFkNodoMenu().getFkVista();

			if (permisoSeguridad.getOperacion() > 4
					|| permisoSeguridad.getOperacion() == 0) {
				VistaOperacionCustom vistaOperacionCustom = vistaOperacionCustomDAO
						.findByVistaAndOperacion(vista.getIdVista(),
								permisoSeguridad.getOperacion());

				if (vistaOperacionCustom != null) {
					operacion.setFkIconSclass(vistaOperacionCustom
							.getFkIconSclass());
					operacion.setFkSclass(vistaOperacionCustom.getFkSclass());
					operacion.setTooltiptext(vistaOperacionCustom
							.getTooltiptext());
					operacion.setNombre(vistaOperacionCustom.getNombre());
				}
			} else {
				VistaOperacionBasico vistaOperacionBasico = vistaOperacionBasicoDAO
						.findByVistaAndOperacion(vista.getIdVista(),
								permisoSeguridad.getOperacion());

				if (vistaOperacionBasico != null) {
					operacion.setFkIconSclass(vistaOperacionBasico
							.getFkIconSclass());
					operacion.setFkSclass(vistaOperacionBasico.getFkSclass());
					operacion.setTooltiptext(vistaOperacionBasico
							.getTooltiptext());
	...
	public String contarHijos(Integer idSesion, Integer idNodoMenuPadre)
			throws Exception {
		auditar(idSesion, getTable(), AccionEnum.CONSULTAR.ordinal(), 0, "");
