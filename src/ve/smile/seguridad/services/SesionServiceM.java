...
public class SesionServiceM extends FachadaService<Sesion> {
	...
	public String pathCerrarSesionWeb(String data) {
		...
		PayloadSesionRequest request = (PayloadSesionRequest) gson.fromJson(
				data, PayloadSesionRequest.class);
	...
	public String pathConsultarPorCriteriosPaginacionWithLastAuditoria(
		...
		try {
			if (validarSesion(idSesion, accessToken)) {
				...
				try {
					typeQuery = TypeQuery.valueOf(multivaluesMapFilters.get(
							"typeQueryToFind").get(0));
				} catch (Exception e) {
				...
				return consultarPaginacionWithLastAuditoria(idSesion, count,
						page, buildSearchFromMap(typeQuery, mapaFiltros));
			}
		} catch (Exception e) {
			return buildAnswerError(e);
		}
	...
	public String consultarPaginacionWithLastAuditoria(Integer idSesion,
			Integer totalElementsByPage, Integer page, Search search)
			throws Exception {
		...
		search.addSort("id" + getClassParameterizedType().getSimpleName(),
				false);
		...
		for (Sesion sesion : sesiones) {
			...
			Auditoria auditoria = auditoriaDAO.findLastAuditoriaOfSesion(sesion
					.getIdSesion());
		...
		String datos = "totalElementsByPage=" + totalElementsByPage + SEPARATOR
				+ "page=" + page;

		for (Filter filter : search.getFilters()) {
			datos += SEPARATOR + filter.getProperty() + " "
		...
		if (sesiones.size() == 0) {
			return buildAnswerInformation(sesiones, INFORMATION_2,
					mapaPaginacion);
		}
